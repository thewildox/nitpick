...
# Reusing one session across two runs is a test-only setup; production uses a
# fresh session per Celery task, so the identity-map collision never occurs.
@pytest.mark.filterwarnings("ignore::sqlalchemy.exc.SAWarning")
def test_analyze_is_idempotent(session):
    repo = Repository(
        github_id=1,
        full_name="thewildox/nitpick-test",
        webhook_secret="testsecret",
    ...
    with patch("app.workers.tasks.fetch_pr_files", return_value=fake_files), \
         ...
         patch("app.workers.tasks.review_snippet", return_value=[]), \
         patch("app.workers.tasks.post_review"):
        analyze_pull_request(run.id, session=session)
        analyze_pull_request(run.id, session=session)   # fire twice
